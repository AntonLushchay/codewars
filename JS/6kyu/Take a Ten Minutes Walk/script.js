...
isValidWalk = (walk) => {
    if (!walk.length || walk.length !== 10) {
        return false;
    }

    const boo = [];

    const obj = walk.reduce((acc, elem) => {
        acc[elem] ? (acc[elem] += 1) : (acc[elem] = 1);
        return acc;
    }, {});
    console.log(obj);

    obj['n'] === obj['s'] ? boo.push(true) : boo.push(false);
    obj['w'] === obj['e'] ? boo.push(true) : boo.push(false);

    return boo.every((elem) => elem === true) ? true : false;
};

console.log(isValidWalk(['s', 'n', 'w', 'e', 'e']));
