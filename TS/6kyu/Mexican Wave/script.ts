...
export function wave(str: string): Array<string> {
    return Array.from(str, createElem(str)).filter(Boolean);
}

const createElem = (str: string): ((val: string, idx: number) => string) => {
    return (val: string, idx: number): string => {
        if (str[idx] && str[idx] !== ' ') {
