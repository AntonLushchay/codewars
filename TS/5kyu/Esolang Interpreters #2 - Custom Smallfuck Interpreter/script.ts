...
export function interpreter(code: string, tape: string): string {
    const commands = ['<', '>', '*', '[', ']'];
    const filtredCode = [...code].filter((val) => commands.includes(val));

    let tapePointer: number = 0;
    let outputArr: Array<string> = [...tape];
    let index: number = 0;

    while (true) {
        const command = filtredCode[index];

        if (index > filtredCode.length - 1) break;

        if (tapePointer > tape.length - 1 || tapePointer < 0) break;
        ...
        if (command === '>') {
            ...
            outputArr[tapePointer] = outputArr[tapePointer] === '0' ? '1' : '0';
            index++;
        } else if (command === '[' && outputArr[tapePointer] === '0') {
            let isCoupleBracketsFound = false;
            let deep = 1;

            for (let i = index + 1; i < filtredCode.length; i++) {
                if (filtredCode[i] === '[') deep++;
                if (filtredCode[i] === ']') deep--;
                if (deep === 0) {
                    index = i;
                    isCoupleBracketsFound = true;
                    break;
                }
            }
            if (!isCoupleBracketsFound) break;
        } else if (command === ']' && outputArr[tapePointer] === '1') {
            let isCoupleBracketsFound = false;
            let deep = 1;
