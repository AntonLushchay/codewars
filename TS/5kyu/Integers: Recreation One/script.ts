...
export const listSquared = (m: number, n: number): number[][] => {
    const output: number[][] = [];
    for (let i = m; i <= n; i++) {
        let sumOfSquaredDivisor = 0;

        for (let j = 1; j <= Math.floor(Math.sqrt(i)); j++) {
            if (i % j === 0) {
                sumOfSquaredDivisor += j * j;
                if (j !== i / j) {
                    sumOfSquaredDivisor += (i / j) * (i / j);
                }
            }
            console.log(`i = ${i}, j = ${j}`);
        }

        if (Number.isInteger(Math.sqrt(sumOfSquaredDivisor))) output.push([i, sumOfSquaredDivisor]);
    }
    return output;
};

console.log(listSquared(250, 600));
